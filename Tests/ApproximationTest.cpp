...
TEST(ApproximationTest, ChangeOfInputVariablesTransformsQuadraticApproximation)
{
    ...
    approximation.dfdxx << 13.0, 17.0,
        19.0, 23.0;
    approximation.dfdux << 1.0, 2.0,
        3.0, 4.0;
    approximation.dfduu << 5.0, 1.0,
        1.0, 3.0;
    ...
    EXPECT_TRUE(approximation.dfdux.isApprox(Pu.transpose() * originalDfdux, 1e-12));
    EXPECT_TRUE(approximation.dfduu.isApprox(Pu.transpose() * originalDfduu * Pu, 1e-12));
    EXPECT_TRUE(approximation.dfdu.isApprox(Pu.transpose() * originalDfdu, 1e-12));
...
TEST(ApproximationTest, ChangeOfInputVariablesTransformsLinearApproximation)
{
    ...
    approximation.dfdx << 1.0, 2.0,
        3.0, 4.0;
    approximation.dfdu << 5.0, 6.0,
        7.0, 8.0;
...
TEST(ApproximationTest, LinearQuadraticApproximatorComputesIntermediateCostAndLQ)
{
    ...
    const auto costApproximation = Approximator::approximateCost(problem, 0.5, state, input);
    Approximator::IntermediateMultiplierCollection_t multipliers;
    const auto modelData = Approximator::approximateIntermediateLQ(problem, 0.5, state, input, multipliers);
...
TEST(ApproximationTest, LinearQuadraticApproximatorComputesFinalCostAndLQ)
{
    ...
    const auto costApproximation = Approximator::approximateFinalCost(problem, 1.5, state);
    const auto modelData = Approximator::approximateFinalLQ(problem, 1.5, state, multipliers);
