...
TEST(iLQREndToEndTest, RunWithQuadraticTrackingCost)
{
    ...
    iLQR<double, 2, 2, 5> solver(&dynamics, &initializer);
    QuadraticStateInputCost<double, 2, 2, 6> runningCost(Eigen::Matrix2d::Identity(), Eigen::Matrix2d::Identity());
...
TEST(iLQREndToEndTest, RolloutMetricsMatchQuadraticTrackingCosts)
{
    ...
    LinearSystemDynamics<double, 2, 2> dynamics(A, B);
    DefaultInitializer<double, 2, 2> initializer;
    Solver solver(&dynamics, &initializer);

    QuadraticStateInputCost<double, 2, 2, 3> runningCost(Eigen::Matrix2d::Identity(), Eigen::Matrix2d::Identity());
    ...
    Solver::computeRolloutMetrics(solver.optimalControlProblem_, primalSolution, dualSolution, metrics);
