...
TEST(LinearSystemILQRTest, IncrementControllerUpdatesOnlyFeedforwardBias)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        EXPECT_DOUBLE_EQ(controller.timeStamp_[i], unoptimizedController.timeStamp_[i]);
        EXPECT_TRUE(controller.gainArray_[i].isApprox(unoptimizedController.gainArray_[i], 1e-12)) << "i = " << i;
        EXPECT_TRUE(controller.biasArray_[i].isApprox(expectedBias, 1e-12)) << "i = " << i;
    }
}
...
TEST(LinearSystemILQRTest, RolloutTrajectoryWritesPrimalSolution)
{
    ...
    initState << 3.0, 4.0;
    const double averageTimeStep = Solver::rolloutTrajectory(rollout, 0.0, initState, 2.0, primalSolution);
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        EXPECT_TRUE(primalSolution.stateTrajectory_[i].isApprox(expectedState, 1e-10)) << "i = " << i;
        EXPECT_TRUE(primalSolution.inputTrajectory_[i].isApprox(expectedInput, 1e-10)) << "i = " << i;
    }
}
...
TEST(TimeTriggeredRolloutTest, ConstantInputMatchesAnalyticSolution)
{
    ...
    RolloutTrajectoryPointer<double, 2, 2> rolloutTrajectoryPointer(
        rolloutTimeTrajectory.data(), rolloutStateTrajectory.data(), rolloutInputTrajectory.data(), 5);

    Eigen::Vector2d initState;
    initState << 3.0, 4.0;
    const int count = rollout.run(0.0, initState, 2.0, &controller, rolloutTrajectoryPointer);
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        EXPECT_TRUE(rolloutStateTrajectory[i].isApprox(expectedState, 1e-10)) << "i = " << i;
        EXPECT_TRUE(rolloutInputTrajectory[i].isApprox(expectedInput, 1e-10)) << "i = " << i;
    }
}
...
TEST(TimeTriggeredRolloutTest, RespectsNonZeroInitialTime)
{
    ...
    Eigen::Vector2d initState;
    initState << 10.0, -10.0;
    const int count = rollout.run(1.0, initState, 2.0, &controller, rolloutTrajectoryPointer);
...
TEST(TimeTriggeredRolloutTest, CanSkipInputTrajectoryReconstruction)
{
    ...
    RolloutTrajectoryPointer<double, 1, 1> rolloutTrajectoryPointer(
        rolloutTimeTrajectory.data(), rolloutStateTrajectory.data(), rolloutInputTrajectory.data(), 3);

    Eigen::Matrix<double, 1, 1> initState;
    initState << 1.0;
    const int count = rollout.run(0.0, initState, 1.0, &controller, rolloutTrajectoryPointer);
