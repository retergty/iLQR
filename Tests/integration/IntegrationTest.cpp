...
TEST(IntegrationTest, IntegratorBaseIntegratesControlledSecondOrderSystem)
{
    ...
    Observer<Scalar, XDim> observer(NumSamples, stateTrajectory.data(), timeTrajectory.data());
...
TEST(IntegrationTest, IntegratorBaseSupportsBackwardIntegration)
{
    ...
    Observer<Scalar, XDim> forwardObserver(NumSamples, forwardStates.data(), forwardTimes.data());
    ...
    Observer<Scalar, XDim> backwardObserver(NumSamples, backwardStates.data(), backwardTimes.data());
    integrator.integrateConst(system, backwardObserver, forwardStates.back(), t1, t0, -dt);
