...
using LinearApproximation = VectorFunctionLinearApproximation<Scalar, XDim, XDim, UDim>;
...
Controller makeConstantController(Scalar t0, Scalar t1, const InputVector& input)
{
...
TEST(SensitivityIntegratorTest, EulerSensitivityMatchesManualConstruction)
{
    ...
    const auto actualApproximation = discretizer.sensitivityDiscretize(system, t, x, u, dt);
...
TEST(SensitivityIntegratorTest, RK2SensitivityMatchesManualConstruction)
{
    ...
    const LinearApproximation k1 = system.linearApproximation(t, x, u);
    const LinearApproximation k2 = system.linearApproximation(t + dt, x + dt * k1.f, u);

    LinearApproximation expected;
    expected.dfdx = StateMatrix::Identity() + dtHalve * k1.dfdx + dtHalve * (k2.dfdx + dt * k2.dfdx * k1.dfdx);
    expected.dfdu = dtHalve * k1.dfdu + dtHalve * (k2.dfdu + dt * k2.dfdx * k1.dfdu);
...
TEST(SensitivityIntegratorTest, RK4SensitivityMatchesManualConstruction)
{
    ...
    const LinearApproximation k1 = system.linearApproximation(t, x, u);
    const LinearApproximation k2 = system.linearApproximation(t + dtHalve, x + dtHalve * k1.f, u);
    const LinearApproximation k3 = system.linearApproximation(t + dtHalve, x + dtHalve * k2.f, u);
    const LinearApproximation k4 = system.linearApproximation(t + dt, x + dt * k3.f, u);
    ...
    LinearApproximation expected;
    expected.dfdx = StateMatrix::Identity() + dtSixth * dk1dxk + dtThird * dk2dxk + dtThird * dk3dxk + dtSixth * dk4dxk;
    expected.dfdu = dtSixth * dk1duk + dtThird * dk2duk + dtThird * dk3duk + dtSixth * dk4duk;
    expected.f = x + dtSixth * k1.f + dtThird * k2.f + dtThird * k3.f + dtSixth * k4.f;
...
TEST(SensitivityIntegratorTest, RK4DiscretizationMatchesDormandPrinceForConstantInput)
{
    System system = makeSystem();
    EK4DynamicsDiscretizer<Scalar, XDim, UDim> discretizer;
    ...
    Observer<Scalar, XDim> observer(2, stateTrajectory.data(), timeTrajectory.data());
    integrator.integrateConst(system, observer, x, t, t + dt, dt);

    const StateVector rk4ForwardDynamics = discretizer.discretize(system, t, x, u, dt);
