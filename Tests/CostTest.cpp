/**
 * @file CostTest.cpp
 * @brief 代价模块测试：匹配当前项目中的 QuadraticStateCost / QuadraticStateInputCost 接口。
 */
#include <array>
...
{
    QuadraticStateInputCost<Scalar, XDim, UDim, ArrayLength> costFunction(Q_, R_, P_);

    const auto cost = costFunction.getValue(t_, x_, u_, timeTrajectory_, stateTrajectory_, inputTrajectory_);
...
TEST_F(QuadraticCostTest, StateInputCostApproximation)
{
    ...
    const auto approximation =
        costFunction.getQuadraticApproximation(t_, x_, u_, timeTrajectory_, stateTrajectory_, inputTrajectory_);

    const StateVector dx = x_ - xNominal_;
    const InputVector du = u_ - uNominal_;

    EXPECT_NEAR(approximation.f, expectedCost_, kPrecision);
    EXPECT_TRUE(approximation.dfdx.isApprox(Q_ * dx + P_.transpose() * du, kPrecision));
...
TEST_F(QuadraticCostTest, StateCostValue)
{
    ...
    const auto cost = costFunction.getValue(t_, x_, timeTrajectory_, stateTrajectory_);
...
TEST_F(QuadraticCostTest, StateCostApproximation)
{
    ...
    const auto approximation = costFunction.getQuadraticApproximation(t_, x_, timeTrajectory_, stateTrajectory_);
