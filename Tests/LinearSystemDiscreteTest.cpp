...
TEST(LinearSystemTest, EK4SensitivityMatchesLinearSystemAnalyticResult)
{
    ...
    const double dt = 0.1;
    const auto approx = ek4Discretizer.sensitivityDiscretize(lin_sys, 0.0, x, u, dt);
    const Eigen::Matrix2d expectedDfdx =
        Eigen::Matrix2d::Identity() + dt * A + 0.5 * dt * dt * A * A + (dt * dt * dt / 6.0) * A * A * A +
        (dt * dt * dt * dt / 24.0) * A * A * A * A;
    const Eigen::Matrix<double, 2, 1> expectedDfdu =
        dt * B + 0.5 * dt * dt * A * B + (dt * dt * dt / 6.0) * A * A * B + (dt * dt * dt * dt / 24.0) * A * A * A * B;
    const Eigen::Vector2d expectedF = ek4Discretizer.discretize(lin_sys, 0.0, x, u, dt);
