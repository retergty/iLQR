...
class Quaternion : public Vector4<Type> {
 public:
   ...
   * @param data_ array
   */
  explicit Quaternion(const Type data_[4]) : Vector4<Type>(data_) {}
   ...
   */
  Quaternion(const Matrix41& other) : Vector4<Type>(other) {}
   ...
   *
   * This sets the instance to a quaternion representing coordinate
   * transformation from frame 2 to frame 1 where the rotation from frame 1 to
   * frame 2 is described by a 3-2-1 intrinsic Tait-Bryan rotation sequence.
   *
   ...
   */
  Quaternion(const Vector3<Type>& src, const Vector3<Type>& dst,
   ...
   * as proposed in
   * [1] Sveier A, Sjøberg AM, Egeland O. "Applied Runge–Kutta–Munthe-Kaas
   * Integration for the Quaternion Kinematics".Journal of Guidance, Control,
   * and Dynamics. 2019
   *
  ...
  static Quaternion expq(const Vector3<Type>& u) {
    ...
    const Type c2 = Type(1.0) / Type(2.0);     // 1 / 2!
    const Type c3 = Type(1.0) / Type(6.0);     // 1 / 3!
    const Type c4 = Type(1.0) / Type(24.0);    // 1 / 4!
    const Type c5 = Type(1.0) / Type(120.0);   // 1 / 5!
    const Type c6 = Type(1.0) / Type(720.0);   // 1 / 6!
    const Type c7 = Type(1.0) / Type(5040.0);  // 1 / 7!
   ...
   * with higher order integration methods (like RK4) on the quaternion
   * logarithm u.
   *
   * @param u 3D vector u
   */
  static Dcm<Type> inv_r_jacobian(const Vector3<Type>& u) {
    ...
    if (u_norm < tol) {  // result smaller than O(||.||^3)
      return Type(0.5) *
             (Dcm<Type>() + u_hat +
              (Type(1.0) / Type(3.0) + u_norm * u_norm / Type(45.0)) * u_hat *
                  u_hat);

    } else {
      return Type(0.5) *
             (Dcm<Type>() + u_hat +
              (Type(1.0) - u_norm * Type(std::cos(u_norm) / std::sin(u_norm))) /
  ...
  Quaternion inversed() const {
    const Quaternion& q = *this;
    Type normSq = q.dot(q);
    return Quaternion(q(0) / normSq, -q(1) / normSq, -q(2) / normSq,
                      -q(3) / normSq);
  }
