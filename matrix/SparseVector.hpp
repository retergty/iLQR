...
class SparseVector {
 private:
  ...
  static_assert(M >= 0, "SparseVector dimension must be non-negative");
  static_assert(!duplicateIndices(), "Duplicate indices");
  static_assert(N < static_cast<size_t>(M),
                "More entries than elements, use a dense vector");
  static_assert(N > 0, "A sparse vector needs at least one element");
  static_assert(findMaxIndex() < static_cast<size_t>(M),
                "Largest entry doesn't fit in sparse vector");
 ...
 public:
  constexpr size_t non_zeros() const { return N; }
  ...
  inline Type at() const {
    static constexpr int compressed_index =
        force_constexpr_eval<findCompressedIndex(i)>::value;
  ...
  matrix::Vector<Type, M> operator+(
      const matrix::Vector<Type, M>& other) const {
...
template <typename Type, int Q, int M, size_t... Idxs>
matrix::Vector<Type, Q> operator*(
    const matrix::Matrix<Type, Q, M>& mat,
...
// returns x.T * A * x
template <typename Type, int M, size_t... Idxs>
Type quadraticForm(const matrix::SquareMatrix<Type, M>& A,
                   const matrix::SparseVector<Type, M, Idxs...>& x) {
  Type res = Type(0);
...
template <typename Type, int M, size_t... Idxs>
constexpr size_t
    SparseVector<Type, M, Idxs...>::_indices[SparseVector<Type, M, Idxs...>::N];

template <int M, size_t... Idxs>
