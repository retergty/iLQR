...
class SquareMatrix : public Matrix<Type, M, M> {
  static_assert(M >= 0, "SquareMatrix dimension must be non-negative");
 ...
 public:
  ...
  void uncorrelateCovariance(size_t first) {
    ...
    SquareMatrix<Type, M>& self = *this;
    Vector<Type, Width> diag_elements =
        self.slice<Width, Width>(first, first).diag();
    self.uncorrelateCovarianceSetVariance(first, diag_elements);
  }

  template <int Width>
  void uncorrelateCovarianceSetVariance(size_t first,
                                        const Vector<Type, Width>& vec) {
    static_assert(Width >= 0, "Block width must be non-negative");
    static_assert(Width <= M, "Width bigger than matrix");
    assert(first + Width <= M);
  ...
  // make block diagonal symmetric by taking the average of the two
    ...
    if (Width > 1) {
      for (size_t row_idx = first + 1; row_idx < first + Width; row_idx++) {
        for (size_t col_idx = first; col_idx < row_idx; col_idx++) {
          Type tmp =
              (self(row_idx, col_idx) + self(col_idx, row_idx)) / Type(2);
  ...
  // make rows and columns symmetric by taking the average of the two
  // corresponding off diagonal values
  template <int Width>
...
bool inv(const SquareMatrix<Type, 1>& A, SquareMatrix<Type, 1>& inv,
         size_t rank = 1) {
