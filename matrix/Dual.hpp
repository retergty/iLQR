...
struct Dual {
  static_assert(N >= 0, "Dual derivative dimension must be non-negative");
  ...
  Dual<Scalar, N>& operator+=(Scalar a) { return (*this = *this + a); }
...
template <typename Scalar, int N>
Dual<Scalar, N> operator*(const Dual<Scalar, N>& a, const Dual<Scalar, N>& b) {
  return Dual<Scalar, N>(a.value * b.value,
                         a.value * b.derivative + b.value * a.derivative);
}
...
Dual<Scalar, N> operator/(const Dual<Scalar, N>& a, const Dual<Scalar, N>& b) {
  ...
  return Dual<Scalar, N>(a.value * inv_b_real,
                         a.derivative * inv_b_real -
...
Dual<Scalar, N> operator/(Scalar a, const Dual<Scalar, N>& b) {
  const Scalar inv_b_real = Scalar(1) / b.value;
  return Dual<Scalar, N>(a * inv_b_real,
                         (-inv_b_real * a * inv_b_real) * b.derivative);
}
...
Dual<Scalar, N> fmod(const Dual<Scalar, N>& a, Scalar mod) {
  return Dual<Scalar, N>(a.value - Scalar(size_t(a.value / mod)) * mod,
                         a.derivative);
}
...
Dual<Scalar, N> atan2(const Dual<Scalar, N>& a, const Dual<Scalar, N>& b) {
  // derivative is equal to that of atan(a/b), so substitute a/b into atan and
  // simplify
  Scalar atan_d = Scalar(1) / (a.value * a.value + b.value * b.value);
  return Dual<Scalar, N>(
      std::atan2(a.value, b.value),
      (a.derivative * b.value - a.value * b.derivative) * atan_d);
}

// retrieve the derivative elements of a vector of Duals into a matrix
template <typename Scalar, int M, int N>
Matrix<Scalar, M, N> collectDerivatives(
    const Matrix<Dual<Scalar, N>, M, 1>& input) {
...
// retrieve the real (non-derivative) elements of a matrix of Duals into an
// equally sized matrix
template <typename Scalar, int M, int N, int D>
...
template <typename OStream, typename Type, int N>
