 ...
 * @tparam XDim 状态维度。
 */
template <typename Scalar, int XDim> class OdeBase {
public:
  /** @brief 默认构造。 */
  OdeBase() = default;
  ...
  computeFlowMap(Scalar t, const Vector<Scalar, XDim> &x) const = 0;
};
 ...
 * @tparam XDim 状态维度。
 */
template <typename Scalar, int XDim> class IntegratorBase {
public:
  /** @brief 默认构造。 */
