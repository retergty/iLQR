...
public:
     ...
     */
    explicit TimeTriggeredRollout(ControlledSystemBase<Scalar, XDim, UDim>* systemDynamics, const Scalar timeStep)
    ...
    /** @brief 返回底层动力学指针。 */
    ControlledSystemBase<Scalar, XDim, UDim>* systemDynamicsPtr() { return systemDynamicsPtr_; }
        ...
        RolloutTrajectoryPointer_t& trajectory) override
    {
        ...
        Observer<Scalar, XDim> observer(trajectory.maxLength, trajectory.stateTrajectory, trajectory.timeTrajectory); // concatenate trajectory
    ...
    // Scalar run(const Scalar initTime, const Vector<Scalar, XDim>& initState, const int steps, ControllerBase<Scalar, XDim, UDim>& controller,
    //     std::array<Scalar, ArrayLen>& timeTrajectory, std::array<Vector<Scalar, XDim>, ArrayLen>& stateTrajectory, std::array<Vector<Scalar, UDim>, ArrayLen>& inputTrajectory) override
    // {
    //     Scalar finalTime = initTime + steps * this->settings().timeStep;
    //     // set controller
    //     systemDynamicsPtr_->setController(&controller);

    //     Observer<Scalar, XDim> observer(ArrayLen, stateTrajectory.data(), timeTrajectory.data()); // concatenate trajectory
    //     // integrate controlled system
    //     RK45Intergraor_.integrateConst(*systemDynamicsPtr_, observer, initState, initTime, finalTime, this->settings().timeStep);
    // }
