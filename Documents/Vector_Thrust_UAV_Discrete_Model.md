...
考虑速度-加速度形式的矢量无人机平动刚体动力学模型。采用 NED 全局坐标系速度与上一拍实际生效的 NED 全局坐标系总加速度构成增广状态：
...
在增量式 MPC 中，状态后 3 维保存控制量记忆。若下游存在限幅或执行器饱和，每次求解时应写入实际发送或估计得到的上一拍 NED 总加速度，避免优化器基于未实际执行的控制量继续累积。
