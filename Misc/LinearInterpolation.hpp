...
namespace LinearInterpolation
{
	...
	/**
	 * @brief 在有序时间数组中查找时间所属区间索引。区间 [t_i, t_{i+1}) 对应区间索引 i。
	 * @param [in] timeArray 有序时间数组。
	 * @param [in] time 查询时间。
	...
	auto interpolate(const std::pair<int, Scalar>& indexAlpha, const std::array<Data, ArrayLength>& dataArray, AccessFun accessFun)
		-> std::decay_t<std::invoke_result_t<AccessFun, const std::array<Data, ArrayLength>&, size_t>>
	{
	...
	auto interpolate(const Scalar enquiryTime, const std::array<Scalar, ArrayLength>& timeArray, const std::array<Data, ArrayLength>& dataArray,
		AccessFun accessFun) -> std::decay_t<std::invoke_result_t<AccessFun, const std::array<Data, ArrayLength>&, size_t>>
	{
	...
	Data interpolate(const Scalar enquiryTime, const std::array<TimeType, ArrayLength>& timeArray, const std::array<Data, ArrayLength>& dataArray)
	{
