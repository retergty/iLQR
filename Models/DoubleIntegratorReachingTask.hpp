...
namespace double_integrator
{
  ...
  public:
    ...
    using QuadraticStateInputCost_t = QuadraticStateInputCost<Scalar, STATE_DIM, INPUT_DIM, ArrayLength>;
    using LinearSystemDynamics_t = LinearSystemDynamics<Scalar, STATE_DIM, INPUT_DIM>;
  ...
  protected:
    ...
    std::unique_ptr<DefaultInitializer_t> getInitializer() const { return std::make_unique<DefaultInitializer_t>(); }
    ...
    std::unique_ptr<StateInputCost_t> getCostPtr() const {
      ...
      return std::make_unique<QuadraticStateInputCost_t>(std::move(Q), std::move(R));
    }
    ...
    std::unique_ptr<StateAugmentedLagrangian> getGoalReachingAugmentedLagrangian(const StateVector_t& xGoal, PenaltyType penaltyType) {
    ...
    public:
      ...
      bool isActive(Scalar time) const override { return referenceManagerPtr_->getModeSchedule().modeAtTime(time) > 0 ? true : false; }

      vector_t getValue(Scalar t, const vector_t& x, const vector_t& u, const PreComputation&) const override { return u; }

      VectorFunctionLinearApproximation getLinearApproximation(Scalar t, const vector_t& x, const vector_t& u,
